...
const onUnlink = depGraph => path => {
  ...
  referrals.forEach(p => {
    ...
    fList.watcher.changeFileList(p, false)
  });
};

const onAdd = path => {
  const errored = Array.from(fList.files.values()).filter(x => x.error && x.error.code === 'Resolving deps');
  // if some files previously failed, retry them, as the newly-added file might be why they failed
  errored.forEach(x => fList.watcher.changeFileList(x.path, false));
};

const depsFor = (fileReflection, canCallDep, usedShims) => {
  ...
  return (nameCleaner, path, source) => {
    ...
    const usesProcess = usedShims.shouldIncludeProcess() ? true : usedShims.shouldExcludeProcess() || isVendor(path) ? false : shims.shouldIncludeProcess(source);
    let deps;
...
const exploreDeps = (depGraph, modMap, usedShims, canCallDep, isProduction, fileReflection, browserResolve) => fileList => {
  ...
  const getDeps = depsFor(fileReflection, canCallDep, usedShims);
